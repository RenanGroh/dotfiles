...
ColumnLayout {
    ...
    Text {
        ...
        // Configurações de Layout (Para alinhar igual ao Relógio)
        Layout.alignment: Qt.AlignHCenter
        Layout.leftMargin: p != "0" ? a == "left" ? -p : a == "right" ? p : 0 : 0
        
        // Estilo
        color: config.MainColor || "white"
        font.pixelSize: config.FontSize ? config.FontSize * 0.4 : 24 // 40% do tamanho da fonte principal
            ...
            onTriggered: {
                ...
                doc.onreadystatechange = function() {
                    if (doc.readyState == XMLHttpRequest.DONE) {
                        weatherLabel.text = doc.responseText.trim();
                    }
                }
                doc.open("GET", "file:///run/sddm-weather.txt");
                doc.send();
            }
        }
    }
